...
class AuthGestionProyectos{
    public $key;
    public function __construct(){
        $this->key = 'dtclavessecreto-9876986986986986s';
    }

    public function moduleLoginGP($empresa,$codigo_acceso,$password,$firebase_token_movil,$firebase_token_web){
        $authJwt = new \App\Helpers\JwtAuth();
        $signup = false; 
        
        $queryModulo = DB::select("SELECT mantenimiento,acceso FROM sos_modulos_sistemas WHERE modulo = 'gestion_proyectos'"); 
            ...
            if ($rMod->mantenimiento == FALSE && $rMod->acceso == TRUE) {
                ...
                if ($signup) {
                    foreach($queryLogin as $user){
                        $permissionLogin = DB::select("SELECT users.login_permission AS login,users.outside_proyectos FROM teci_catalogo_usuarios AS users JOIN main_empresas AS emp 
                        WHERE emp.id = users.empresa AND (users.codigo_acceso = ? OR users.username = ?) 
                        AND users.password = ?",[$codigo_acceso,$codigo_acceso,$password]);

                        if (end($permissionLogin)->login == TRUE && end($permissionLogin)->outside_proyectos == TRUE) {
                            ...
                            foreach ($infoUser as $rUser) {
                                ...
                                if (count($histBitacora) == 0) {
                                    ...
                                    if (count($dataEmpresa) > 0) {
                                        $enlaceLink = './gestion_de_proyectos/home';
                                ...
                                $dataMensaje = array(
                                    ...
                                    "modulo_code" => "ZUpOYkRNby81M2ZaT0ozYjlmb0srL2FwcCtxa0ZUY1N3TUYyU3F1Y05RRT06OjEyMzQ1Njc4MTIzNDU2Nzg=",
                                    "settings_privilegio_crear" => $privilegio_crear,
                                    "settings_privilegio_editar" => $privilegio_editar,
                                    "settings_privilegio_consulta" => $privilegio_consulta,
                                    "settings_privilegio_elimina" => $privilegio_elimina,
    ...
    public function signupNewPass($user_token,$passPrimera,$passSegunda,$passOlder){
        ...
        $queryVerif = DB::select("SELECT users.user_token,emp.empresa_token,emp.root_tkn FROM teci_catalogo_usuarios AS users JOIN main_empresas AS emp 
        ...
        if ($signup) {
            foreach($queryVerif as $user){
                $permissionLogin = DB::select("SELECT users.login_permission AS login,users.outside_proyectos FROM teci_catalogo_usuarios AS users JOIN main_empresas AS emp 
                    WHERE emp.empresa_token = ? AND emp.id = users.empresa AND users.user_token = ? AND users.password = ?",
                [$user->empresa_token,$user->user_token,$passOlder]);
                //echo $permissionLogin[0]->id_permisos_usuario." ".$permissionLogin[0]->login;
                if ($permissionLogin[0]->login == TRUE && $permissionLogin[0]->outside_proyectos == TRUE) {
                    ...
                    if ($passPrimera == $passSegunda) {
                        ...
                        if ($userUpdate) {
                            ...
                            $selectEmp = DB::select("SELECT emp.id,users.id AS userId,emp.zona_horaria,emp.root_tkn FROM main_empresas AS emp
                                JOIN main_empresapersonal AS emppers JOIN vhum_empleados_catalogo AS pers JOIN teci_catalogo_usuarios AS users WHERE emp.empresa_token = ?
                                AND emp.id = emppers.empresa AND emppers.personal = pers.id
                                AND pers.usuario = users.id AND users.user_token= ?",[$user->empresa_token,$user_token]);
                            //echo $user_token;
                            ...
                            if ($insertBitacora) {
                                ...
                                $dataMensaje = array(
                                    ...
                                    "modulo_destino" => $enlaceLink,
                                    "validate_process" => $validate_process,
                                    "modulo_title" => "Gestión de proyectos",
                                    "modulo_code" => "ZUpOYkRNby81M2ZaT0ozYjlmb0srL2FwcCtxa0ZUY1N3TUYyU3F1Y05RRT06OjEyMzQ1Njc4MTIzNDU2Nzg=",
                                    "settings_privilegio_crear" => $privilegio_crear,
                                    "settings_privilegio_editar" => $privilegio_editar,
                                    "settings_privilegio_consulta" => $privilegio_consulta,
