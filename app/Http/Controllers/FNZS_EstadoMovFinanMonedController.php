...
class FNZS_EstadoMovFinanMonedController extends Controller{
  private function saldoInicialMonedero($token_cuentamonedero,$empresa,$fechaInicio){
  ...
  public function movimientosFinancierosMonederoElectronico(Request $request){
    ...
    if ($validate->fails()) {
      ...
      $token_cuentamonedero = $request->input('token_cuentamonedero');
      ...
      if ($queryMovimientos->isEmpty()) {
        ...
        $saldo_inicial_monedero = $this->saldoInicialMonedero($token_cuentamonedero,$empresa,$fechaInicio);
        $saldo_acumulado_depositos = 0;
        $saldo_acumulado_retiros = 0;
        $saldo_acumulado_progresivo = $saldo_inicial_monedero;
        $contador = 0;
        ...
        foreach ($queryMovimientos as $vMov) {
          ...
          $token_cuentamonedero = $vMov->token_cuentamonedero;
        ...
        $cuenta_result_saldo = ($saldo_inicial_monedero + $saldo_acumulado_depositos) - $saldo_acumulado_retiros;
        ...
        $dataMensaje = array(
          ...
          "movimientos_saldo_inicial" => "$".number_format($saldo_inicial_monedero,$decimalesMoneda,'.', ','),
