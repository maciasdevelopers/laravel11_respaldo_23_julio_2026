...
class INGR_FacturacionController extends Controller{
    
    public function solicitudesCFDI(Request $request){
        $JwtAuth = new \JwtAuth();
        ...
        if (!empty($parametros) && !empty($parametrosArray)) {
            ...
            if ($validate->fails()) {
                ...
                if ($usuario->user_token == "ZnRNZzFSSUQ1OE1VM0hYNkxZTjEyQT09OjoxMjM0NTY3ODEyMzQ1Njc4" || 
                    $usuario->user_token == "") {
                    $listSoliCFDI = CfdiModelo::join("cfdi_solicitud AS soli_cfdi","cfdi_main.id","=","soli_cfdi.cfdi_main")
                    ->join("empresas AS emp","cfdi_main.receptor","=","emp.id")
                    ->where([
                        "soli_cfdi.valid_version" => TRUE,
                        "cfdi_main.registro_cfdi" => "so",
                        "cfdi_main.status_cfdi" => TRUE,
                        "emp.emp_token" => $usuario->emp_token
                    ])                
                    ->orderBy('cfdi_main.folio_cfdi','DESC')->get();
                } else {
                    $listSoliCFDI = CfdiModelo::join("cfdi_solicitud AS soli_cfdi","cfdi_main.id","=","soli_cfdi.cfdi_main")
                    ->join("empresas AS emp","cfdi_main.receptor","=","emp.id")
                    ->join("personal AS pers","cfdi_main.user_receptor","=","pers.id")
                ...
                foreach ($listSoliCFDI as $vsoli) {
                    ...
                    $selectNameEmpEmi = CfdiModelo::join("empresas AS emp","cfdi_main.emisor","=","emp.id")
                    ->join("personas AS people","emp.persona","=","people.id")
                    ->where([
                        "cfdi_main.token_cfdi" => $token_cfdi,
                    ])->get();
                    ...
                    $selectPersEmpEmi = CfdiModelo::join("empresas AS emp","cfdi_main.emisor","=","emp.id")
                    ->join("personal AS pers","cfdi_main.user_emisor","=","pers.id")
                    ->join("personas AS people","pers.personal","=","people.id")
                    ->where(["cfdi_main.token_cfdi" => $token_cfdi])->get();
                    ...
                    if ($vsoli->cliente != NULL) {
                        $selectNameEmpFact = CfdiModelo::join("cfdi_solicitud AS soli_cfdi","cfdi_main.id","=","soli_cfdi.cfdi_main")
                        ->join("catalogo_clientes AS cKli","soli_cfdi.cliente","=","cKli.id")
                        ->join("personas AS client","cKli.cliente","=","client.id")
                        ->join("empresas AS emp","cfdi_main.emisor","=","emp.id")
                        ->join("personal AS pers","cfdi_main.user_emisor","=","pers.id")
                        ->join("usuarios AS users","pers.usuario","=","users.id")
                        ->where(["cfdi_main.token_cfdi" => $token_cfdi])->get();
                    ...
                    $selectDetCFDI = DB::table("cfdi_detalle_solicitud AS det_soli")
                    ->join("cfdi_solicitud AS soli","det_soli.solicitud_cfdi","=","soli.id")
                    ->join("cfdi_main AS row","det_soli.cfdi_main","=","row.id")
                    ->where([
                        "row.token_cfdi" => $token_cfdi,
                    ...
                    foreach ($selectDetCFDI as $vDet) {
                        ...
                        if ($vDet->clave_sat != NULL) {
                            $satKeyDetCFDI = DB::table("cfdi_detalle_solicitud AS det_soli")
                            ->join("catalogo_prodservsat AS sat","det_soli.clave_sat","=","sat.id")
                            ->join("cfdi_solicitud AS soli","det_soli.solicitud","=","soli.id")
                            ->where([
                        ...
                        if ($vDet->unidad_medida != NULL) {
                            $satMedDetCFDI = DB::table("cfdi_detalle_solicitud AS det_soli")
                            ->join("unidad_medida AS med","det_soli.unidad_medida","=","med.id")
                            ->join("cfdi_solicitud AS soli","det_soli.solicitud_cfdi","=","soli.id")
                            ->where([
                                "det_soli.token_detalle_soli" => $token_det_soli,
                                "soli.token_solicitud_cfdi" => $tkn_soli_cfdi,
                        ...
                        $fPagoDetCFDI = DB::table("cfdi_detalle_solicitud AS det_soli")
                        ->join("forma_pago AS fpago","det_soli.forma_pago","=","fpago.id")
                        ->join("cfdi_solicitud AS soli","det_soli.solicitud_cfdi","=","soli.id")
                        ->where([
                            "det_soli.token_detalle_soli" => $token_det_soli,
                            "soli.token_solicitud_cfdi" => $tkn_soli_cfdi,
                        ...
                        $mPagoDetCFDI = DB::table("cfdi_detalle_solicitud AS det_soli")
                        ->join("metodo_pago AS mpago","det_soli.metodo_pago","=","mpago.id")
                        ->join("cfdi_solicitud AS soli","det_soli.solicitud_cfdi","=","soli.id")
                        ->where([
                            "det_soli.token_detalle_soli" => $token_det_soli,
                            "soli.token_solicitud_cfdi" => $tkn_soli_cfdi,
                        ...
                        if ($vDet->uso_cfdi != NULL) {
                            $usoCDetCFDI = DB::table("cfdi_detalle_solicitud AS det_soli")
                            ->join("uso_cfdi AS uso","det_soli.uso_cfdi","=","uso.id")
                            ->join("cfdi_solicitud AS soli","det_soli.solicitud_cfdi","=","soli.id")
                            ->where([
                                "det_soli.token_detalle_soli" => $token_det_soli,
                                "soli.token_solicitud_cfdi" => $tkn_soli_cfdi,
                    ...
                    $soli_cancelaciones = CfdiModelo::join("cfdi_cancelaciones AS soli_can","cfdi_main.id","=","soli_can.cfdi_main")
                    ...
                    $pendientes = count($soli_cancelaciones);
        ...
        if (!empty($parametros) && !empty($parametrosArray)) {
            ...
            if ($validate->fails()) {
                ...
                if ($usuario->user_token == "ZnRNZzFSSUQ1OE1VM0hYNkxZTjEyQT09OjoxMjM0NTY3ODEyMzQ1Njc4" || 
                    ...
                    $cfdi_main_selected = CfdiModelo::join("empresas AS emp","cfdi_main.receptor","=","emp.id")
                    ->where([
                        "cfdi_main.token_cfdi" => $token_cfdi,
                        "cfdi_main.registro_cfdi" => "so",
                        "cfdi_main.status_cfdi" => TRUE,
                ...
                foreach ($cfdi_main_selected as $vsoli) {
                    ...
                    //emisor
                        $selectNameEmpEmi = CfdiModelo::join("empresas AS emp","cfdi_main.emisor","=","emp.id")
                        ->join("personas AS people","emp.persona","=","people.id")
                        ->where(["cfdi_main.token_cfdi" => $vsoli->token_cfdi])->get();
                
                        foreach ($selectNameEmpEmi as $vEmisor) {
                            $tkn_emisor = $vEmisor->emp_token;
                            if ($vEmisor->denominacion_rs == '') {
                                $name_emisor = $JwtAuth->desencriptar($vEmisor->paterno).
                                    " ".$JwtAuth->desencriptar($vEmisor->materno).
                                    " ".$JwtAuth->desencriptar($vEmisor->nombre);
                        ...
                        $selectPersEmpEmi = CfdiModelo::join("personal AS pers","cfdi_main.user_emisor","=","pers.id")
                        ->join("personas AS people","pers.personal","=","people.id")
                        ->where(["cfdi_main.token_cfdi" => $vsoli->token_cfdi])->get();
                    ...
                    //receptor 
                        $selectNameEmpRec = CfdiModelo::join("empresas AS emp","cfdi_main.receptor","=","emp.id")
                        ->join("personas AS people","emp.persona","=","people.id")
                        ->where(["cfdi_main.token_cfdi" => $vsoli->token_cfdi])->get();
                        ...
                        $selectPersEmpReceptor = CfdiModelo::join("personal AS pers","cfdi_main.user_receptor","=","pers.id")
                        ->join("personas AS people","pers.personal","=","people.id")
                        ->where(["cfdi_main.token_cfdi" => $vsoli->token_cfdi])->get();
                    ...
                    //last_version
                        $soli_last_version = CfdiModelo::join("cfdi_solicitud AS soli_cfdi","cfdi_main.id","=","soli_cfdi.cfdi_main")
                        ->join("catalogo_clientes AS cKli","soli_cfdi.cliente","=","cKli.id")
                        ->join("personas AS client","cKli.cliente","=","client.id")
                        ->where([
                            "cfdi_main.token_cfdi" => $vsoli->token_cfdi,
                            "cfdi_main.registro_cfdi" => "so",
                            "cfdi_main.status_cfdi" => TRUE,
                            "soli_cfdi.valid_version" => TRUE,
                        ...
                        foreach ($soli_last_version as $vLast) {
                            ...
                            ->join("cfdi_main AS crow","docs.cfdi_main","=","crow.id")
                            ->join("cfdi_solicitud AS soli","docs.solicitud_cfdi","=","soli.id")
                            ->where([
                                "docs.tipo_doc" => "an",
                                "crow.token_cfdi" => $token_cfdi,
                                "soli.token_solicitud_cfdi" => $tkn_solicitud_cfdi,
                            ])->get();
                    ...
                    //old_versions
                        $soli_old_versions = CfdiModelo::join("cfdi_solicitud AS soli_cfdi","cfdi_main.id","=","soli_cfdi.cfdi_main")
                        ->join("catalogo_clientes AS cKli","soli_cfdi.cliente","=","cKli.id")
                        ->join("personas AS client","cKli.cliente","=","client.id")
                        ->where([
                            "cfdi_main.token_cfdi" => $vsoli->token_cfdi,
                    ...
                    //solicitud_cancelaciones
                        ...
                        foreach ($soli_cancelaciones as $vCan) {
                            ...
                            if ($vCan->solicitud_cfdi_old != NULL) {
                                ...
                                ->join("cfdi_solicitud AS soli_cfdi","cancel.solicitud_cfdi_old","=","soli_cfdi.id")
                            ...
                            if ($vCan->solicitud_cfdi_new != NULL) {
                                ...
                                ->join("cfdi_solicitud AS soli_cfdi","cancel.solicitud_cfdi_new","=","soli_cfdi.id")
                                ->where(["cancel.token_cancelacion" => $tkn_cancelacion])->get();
                    ...
                    //cancelaciones_autorizadas
                        $auth_cancelaciones = CfdiModelo::join("cfdi_cancelaciones AS soli_can","cfdi_main.id","=","soli_can.cfdi_main")
                        ->join("cfdi_motivos_cancelacion AS m_can","soli_can.motivo_cancelacion","=","m_can.id")
                        ->where([
        ...
        if (!empty($parametros) && !empty($parametrosArray)) {
            ...
            if ($validate->fails()) {
                ...
                if (isset($token_cfdi) && !empty($token_cfdi) &&
                    ...
                    //return response()->json(['status' => 'error','code' => 200,'message' => "cfdi_solicitud_01"]);     
                    $select_cfdi_main = DB::select("SELECT id,folio_cfdi,post_folio_cfdi FROM cfdi_main WHERE token_cfdi = ?",[$token_cfdi]); 
                    ...
                    //return response()->json(['status' => 'error','code' => 200,'message' => "cfdi_solicitud_02"]); 
                    $first_soli_cfdi = DB::select("SELECT csol.id,csol.folio_solicitud+1 AS folio_solicitud FROM cfdi_solicitud AS csol 
                        JOIN cfdi_main AS crow WHERE csol.token_solicitud_cfdi = ? 
                        AND csol.cfdi_main = crow.id AND crow.token_cfdi = ?",[$token_solicitud_cfdi,$token_cfdi]);
                    $second_soli_cfdi = "";    
                    //return response()->json(['status' => 'error','code' => 200,'message' => "cfdi_solicitud_03"]);
                    ...
                    $queryAllCancelat = DB::select("SELECT ccanc.id FROM cfdi_cancelaciones AS ccanc 
                        JOIN cfdi_main AS crow WHERE ccanc.cfdi_main = crow.id AND crow.token_cfdi = ?",[$token_cfdi]);
                    //return response()->json(['status' => 'error','code' => 200,'message' => "cfdi_solicitud_04"]);
                    if (count($queryAllCancelat) == 0) {
                        $folio_canc_nuevo = 1;
                    } else {
                    ...
                    //return response()->json(['status' => 'error','code' => 200,'message' => "cfdi_solicitud_05"]);
                    $folio_canc_all = $JwtAuth->generarFolio($folio_canc_nuevo);
                    //return response()->json(['status' => 'error','code' => 200,'message' => "cfdi_solicitud ".$folio_canc_all]);
                    if ($clave_cancelacion == "01" || $clave_cancelacion == "04") {
                        ...
                        if (isset($client_tkn_soli) && !empty($client_tkn_soli) &&
                            ...
                            //return response()->json(['status' => 'error','code' => 200,'message' => "cfdi_solicitud_06"]);     
                            ...
                            //return response()->json(['status' => 'error','code' => 200,'message' => "cfdi_solicitud_07"]);
                            $selectEmisorPers = DB::select("SELECT pers.id FROM personal AS pers 
                                JOIN usuarios AS users WHERE pers.usuario = users.id AND users.user_token = ?",
                                [$usuario->user_token]);
                            //return response()->json(['status' => 'error','code' => 200,'message' => "cfdi_solicitud_08"]);
                            foreach ($selectEmisorPers as $vPersEmi) {
                                $emisor_pers = $vPersEmi->id;
                            }
                            //return response()->json(['status' => 'error','code' => 200,'message' => "cfdi_solicitud_09"]);
                            $selectEmpIdFact = DB::select("SELECT id FROM catalogo_clientes 
                                WHERE token_cat_clientes = ?",[$client_tkn_soli]);
                            
                            foreach ($selectEmpIdFact as $vEmpSoli) {
                                $clienteIdent = $vEmpSoli->id;
                            }
                            //return response()->json(['status' => 'error','code' => 200,'message' => "cfdi_solicitud_10"]);
                            ...
                            //return response()->json(['status' => 'error','code' => 200,'message' => "cfdi_solicitud_11"]);
                            if ($nacionalidad == 118) {
                            ...
                            //return response()->json(['status' => 'error','code' => 200,'message' => "cfdi_solicitud_12"]);
                            ...
                            //return response()->json(['status' => 'error','code' => 200,'message' => "cfdi_solicitud_13"]);
                            ...
                            //return response()->json(['status' => 'error','code' => 200,'message' => "cfdi_solicitud_14"]);    
                            $new_folio_solicitud = $JwtAuth->generarFolio($first_soli_cfdi[0]->folio_solicitud);
                            //return response()->json(['status' => 'error','code' => 200,'message' => "cfdi_solicitud_15"]);  
                            //return response()->json(['status' => 'error','code' => 200,'message' => "cfdi_solicitud_15 ".$new_folio_solicitud]);
                            
                            $falseOlderVersions = DB::table('cfdi_solicitud AS csol')
                            ->join("cfdi_main AS crow","csol.cfdi_main","=","crow.id")
                            ->where([
                                'csol.token_solicitud_cfdi' => $token_solicitud_cfdi,
                                'crow.token_cfdi' => $token_cfdi,
                            ])->limit(1)->update(array('csol.valid_version' => FALSE));
                            ...
                            if ($insert_soli_cfdi) {
                                //return response()->json(['status' => 'error','code' => 200,'message' => "cfdi_solicitud_16"]);  
                                $query_soli_cfdi = DB::select("SELECT id FROM cfdi_solicitud WHERE token_solicitud_cfdi = ?",[$token_soli_cfdi]);
                                ...
                                for ($i = 0; $i < count($soliCfdiXmlSoli); $i++) {
                                    ...
                                    if ($sat_token != "") {
                                        $selectClaveSat = DB::select("SELECT id FROM catalogo_prodservsat 
                                    ...
                                    if ($unidad_medida != "") {
                                        $selectUMedida = DB::select("SELECT id FROM unidad_medida 
                                    ...
                                    $selectFPago = DB::select("SELECT id FROM forma_pago 
                                        WHERE token_formapago = ?",[$forma_pago]);
                                    $sql_f_pago = $selectFPago[0]->id;
                                    
                                    $selectMPago = DB::select("SELECT id FROM metodo_pago 
                                        WHERE token_metodopago = ?",[$metodo_pago]);
                                    $sql_m_pago = $selectMPago[0]->id;
                                    
                                    if ($uso_cfdi != "") {
                                        $selectUsoCFDI = DB::select("SELECT id FROM uso_cfdi 
                                            WHERE token_uso_cfdi = ?",[$uso_cfdi]);
                                        $sql_uso_cfdi = $selectUsoCFDI[0]->id;
                                    } else {
                                        $sql_uso_cfdi = NULL;
                                    }
                                    //return response()->json(['status' => 'error','code' => 200,'message' => "cfdi_solicitud_17"]);  
                                ...
                                //return response()->json(['status' => 'error','code' => 200,'message' => "cfdi_solicitud_19"]);
                                if(!empty($_FILES["docsAnexos"])){
                                    $anexos = $_FILES["docsAnexos"];
                                    //return response()->json(['status' => 'error','code' => 200,'message' => "cfdi_solicitud_21"]);
                                ...
                                //return response()->json(['status' => 'error','code' => 200,'message' => "cfdi_solicitud_20"]);
                            } else {
                                $dataMensaje = array(
                                    'status' => 'error',
                                    'code' => 200,
            ...
            if ($validate->fails()) {
                ...
                if (isset($client_tkn_soli) && !empty($client_tkn_soli) &&
                    ...
                    $folioSistema = DB::select("SELECT fold.folder+1 AS folio,fold.post_folder
                        FROM last_folders AS fold JOIN empresas AS emp JOIN empresapersonal AS emppers 
                        JOIN personal AS pers JOIN usuarios AS users WHERE fold.solicitud_cfdi = TRUE 
                    ...
                    if (count($folioSistema) == 1) {
                        if ($folioSistema[0]->folio == 1000000000) {
                            $post_folio_db = DB::select("SELECT post_folio_cfdi FROM cfdi_main 
                                WHERE id = (SELECT Max(soli.id) FROM cfdi_main AS soli 
                    ...
                    if ($insertCfdi) {
                        $select_cfdi_main = DB::select("SELECT id FROM cfdi_main WHERE token_cfdi = ?",[$token_cfdi_main]);
                        $token_soli_cfdi = $JwtAuth->encriptarToken($client_tkn_soli.$soliCfdiRfc.
                            $soliCfdiEmp.$soliCfdiEmail.$emisor_emp.$emisor_pers.$clienteIdent);
                        ...
                        if ($insert_soli_cfdi) {
                            ...
                            if (count($folioSistema) == 0) {
                                ...
                                $regFolder = DB::table('last_folders')->join("empresas AS emp","last_folders.empresa","=","emp.id")
    ...
    public function emisionCFDI(Request $request){
        $JwtAuth = new \JwtAuth();
        $docsAnexos = $request->file("docsAnexos");
        $jsonUser = $request->input('json');
        $parametros = json_decode($jsonUser);
        ...
        if (!empty($parametros) && !empty($parametrosArray)) {
            ...
            if ($validate->fails()) {
                ...
                $token_cfdi = $parametrosArray["token_cfdi"];
                $token_solicitud_cfdi = $parametrosArray["token_solicitud_cfdi"];
                //$fecha_emision = $parametrosArray["fecha_emision"];
                $e_f_emision = explode("T", $parametrosArray["fecha_emision"]);
                $fecha_emision = $e_f_emision[0];
                //echo $soliCfdiRfc;exit;
                $fecha_sistema = time();
                $patronFecha = '/^[0-9-]*$/';
                ...
                if (isset($token_cfdi) && !empty($token_cfdi) &&
                    isset($token_solicitud_cfdi) && !empty($token_solicitud_cfdi) && 
                    isset($fecha_emision) && !empty($fecha_emision) && preg_match($patronFecha,$fecha_emision) &&
                    isset($_FILES["docsAnexos"]) && !empty($_FILES["docsAnexos"])) {
                    $selectEmp = DB::select("SELECT emp.id,emp.zona_horaria,emp.root_tkn FROM empresas AS emp  
                        JOIN cfdi_main AS crow WHERE emp.id = crow.emisor AND crow.token_cfdi = ?",[$token_cfdi]);
                        //da_te_default_timezone_set($selectEmp[0]->zona_horaria); 
                        
                    $queryAllEmisiones = DB::select("SELECT cemi.id FROM cfdi_emision AS cemi 
                        JOIN cfdi_main AS crow WHERE cemi.cfdi_main = crow.id AND crow.token_cfdi = ?",[$token_cfdi]);
                    //return response()->json(['status' => 'error','code' => 200,'message' => "cfdi_solicitud_04"]);
                    if (count($queryAllEmisiones) == 0) {
                        $folio_emi_nuevo = 1;
                    } else {
                        $queryMaxEmision = DB::select("SELECT folio_sistema_emision+1 AS folio FROM cfdi_emision
                            WHERE id = (SELECT MAX(cemi.id) FROM cfdi_emision AS cemi JOIN cfdi_main AS crow 
                            WHERE cemi.cfdi_main = crow.id AND crow.token_cfdi = ?)",[$token_cfdi]);
                        $folio_emi_nuevo = $queryMaxEmision[0]->folio;
                    }
                    //return response()->json(['status' => 'error','code' => 200,'message' => "cfdi_solicitud_05"]);
                    $folio_emi_all = $JwtAuth->generarFolio($folio_emi_nuevo);
                    
                    $select_cfdi_main = DB::table("cfdi_main")->where(["token_cfdi" => $token_cfdi])->get();
                    
                    foreach ($select_cfdi_main as $cfdi) {
                        $cfdi_id = $cfdi->id;
                        if ($cfdi->post_folio_cfdi == NULL) {
                            $folio_cfdi = 'CFDI-'.$JwtAuth->generarFolio($cfdi->folio_cfdi);
                        } else {
                            $folio_cfdi = 'CFDI-'.$JwtAuth->generarFolio($cfdi->folio_cfdi).'-'.$cfdi->post_folio_cfdi;
                        } 
                    } 
                    //return response()->json(['status' => 'error','code' => 200,'message' => "cfdi_solicitud_06"]);
                    $query_soli_cfdi = DB::select("SELECT csol.id,csol.folio_solicitud+1 AS folio_solicitud FROM cfdi_solicitud AS csol 
                        JOIN cfdi_main AS crow WHERE csol.token_solicitud_cfdi = ? 
                        AND csol.cfdi_main = crow.id AND crow.token_cfdi = ?",
                        [$token_solicitud_cfdi,$token_cfdi]);
                    $new_folio_solicitud = $JwtAuth->generarFolio($query_soli_cfdi[0]->folio_solicitud);
                    //return response()->json(['status' => 'error','code' => 200,'message' => "cfdi_solicitud_07"]);    
                    $token_emision = $JwtAuth->encriptarToken($token_cfdi.$token_solicitud_cfdi.$fecha_sistema.$folio_emi_all.
                        $cfdi_id.$query_soli_cfdi[0]->id);
                    //return response()->json(['status' => 'error','code' => 200,'message' => "cfdi_solicitud_08 ".$JwtAuth->convierteFechaEpoc($fecha_emision)]);    
                    $insert_emision = DB::table('cfdi_emision')->insert(
                        array(
                            "token_emision" => $token_emision, 
                            "folio_sistema_emision" => $folio_emi_nuevo,
                            "fecha_sistema_emision" => $fecha_sistema, 
                            "cfdi_main" => $cfdi_id,
                            "solicitud_cfdi" => $query_soli_cfdi[0]->id,
                            "fecha_emision" => $JwtAuth->convierteFechaEpoc($fecha_emision),
                            "status_emision" => TRUE,
                            "fecha_delete" => NULL
                        )
                    );
                    ...
                    if ($insert_emision) {
                        $query_emision = DB::select("SELECT id FROM cfdi_emision WHERE token_emision = ?",[$token_emision]);
                        //return response()->json(['status' => 'error','code' => 200,'message' => "cfdi_solicitud_09".$selectEmp[0]->root_tkn]);
                        $filepath = $selectEmp[0]->root_tkn."/0009-cfdi/".$folio_cfdi."/version-".$new_folio_solicitud."/emision/".$folio_emi_all."/";
                        //return response()->json(['status' => 'error','code' => 200,'message' => "cfdi_solicitud_09"]); 
                        if (!file_exists(storage_path("/root/".$filepath))){
                            Storage::disk('root')->makeDirectory($filepath,0777, true, true);
                        }
                        ...
                        for ($i=0; $i < count($docs_nombre); $i++){
                            ...
                            if ($type == "application/pdf") {
                                ...
                                $ext_doc = "xml";
                                //return response()->json(['status' => 'error','code' => 200,'message' => "cfdi_solicitud_09".$name_documento]);
                            }
                            
                            $temporal = $anexos["tmp_name"][$i];
                            //return response()->json(['status' => 'error','code' => 200,'message' => "cfdi_solicitud_09".$name_documento]);
                            $token_docs = $JwtAuth->encriptarToken($token_cfdi,$token_solicitud_cfdi,$ext_doc,$name_documento);
                            //return response()->json(['status' => 'error','code' => 200,'message' => $JwtAuth->encriptar($name_documento)]);
                            $insertDocResp = DB::table("cfdi_docs")->insert(
                                array(
                                    "token_doc_cfdi" => $token_docs,
                                    "cfdi_main" => $cfdi_id,
                                    "solicitud_cfdi" => $query_soli_cfdi[0]->id,
                                    "emision_cfdi" => $query_emision[0]->id,
                                    "tipo_doc" => "re",
                                    "ext_doc" => $ext_doc,
                                    "name_documento" => $JwtAuth->encriptar($name_documento),
                                )
                            ...
                            if ($insertDocResp) {
                                $contDocs++;
                                Storage::putFileAs("/public/root/".$filepath,$temporal,$name_documento);
                                //return response()->json(['status' => 'error','code' => 200,'message' => $contDocs]);
                            }
                        }
                        //return response()->json(['status' => 'error','code' => 200,'message' => $contDocs." == ".count($docs_nombre)]);
                        if ($contDocs == count($docs_nombre)) {
                            $dataMensaje = array(
                                'status' => 'success',
                                'code' => 200,
                                'message' => 'Emision de CFDI exitosa'
                            );
                        } else {
                            $dataMensaje = array(
                                'status' => 'error',
                                'code' => 200,
                                'message' => 'Error en emision de cfdi, verifique su información'
                            );
                        }
                    }    
                    ...
                    if (!isset($fecha_emision) || empty($fecha_emision) || !preg_match($patronFecha,$fecha_emision)) {
                        $dataMensaje = array(
                            'status' => 'error',
                            'code' => 200,
                            'message' => 'Error en fecha de emisión de cfdi seleccionado, verifique su información'
                        );
                    }
                    if (!isset($_FILES["docsAnexos"]) || empty($_FILES["docsAnexos"])) {
                        $dataMensaje = array(
                            'status' => 'error',
                            'code' => 200,
                            'message' => 'Error en documentos adjuntos, verifique su información'
                        );
                    }
                }
