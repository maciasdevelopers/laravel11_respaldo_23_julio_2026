    ...
    if ($validate->fails()) {
      ...
      if ($queryMovimientos->isEmpty()) {
        ...
        foreach ($queryMovimientos as $vMov) {
          ...
          $fecha_contabilizacion = " date ".date('Y-m-d H:i:s', $mov_f_cont)." gmdate ".gmdate('Y-m-d H:i:s', $mov_f_cont);
          $correct_fecha_cont = $JwtAuth->corregirTimestampUnixHistorico($mov_f_cont);
          ...
          if (!is_null($vMov->pago)) {
            //$query_movimeintos_cp = DB::table("fnzs_actividad_movimientos AS act_moviment")
            //->join("fnzs_pagos_pago AS pay", "act_moviment.pago", "=", "pay.id")
            //->where('act_moviment.pago',$vMov->pago)
            //->orderBy("act_moviment.id","DESC")
            //->select('act_moviment.id AS id_movim','act_moviment.fecha_contabilizacion_movimiento_old AS fecha_movim','act_moviment.fecha_contabilizacion_cancelacion_old AS cancel_movim')
            //->get();
      
            //foreach ($query_movimeintos_cp as $lMov) {
            //  $correct_fecha_contabilizacion = !is_null($lMov->fecha_movim) ? $JwtAuth->corregirTimestampUnixHistorico($lMov->fecha_movim): null;
            //  $correct_canceled_fecha = !is_null($lMov->cancel_movim) ? $JwtAuth->corregirTimestampUnixHistorico($lMov->cancel_movim): null;
            //  DB::table("fnzs_actividad_movimientos")
            //  ->where('id',$lMov->id_movim)
            //  ->limit(1)->update(array( 
            //    "fecha_contabilizacion_movimiento" => $correct_fecha_contabilizacion, 
            //    "fecha_contabilizacion_cancelacion" => $correct_canceled_fecha
            //  ));
            //}
