...
class FNZS_EstadoMovFinanClienteController extends Controller{
  private function saldoInicialClienteByToken($token_cat_clientes,$empresa,$fechaInicio){
    $saldo_inicial_progresivo = 0;
    $movimientosClienteByCompraOrdenPago = MovimientosBancariosModelo::join("main_empresas AS emp", "fnzs_actividad_movimientos.empresa", "=", "emp.id")
    ...
    ->join("ingr_ventas AS sell", "order.factura_venta", "=", "sell.id")
    ->join("ingr_catalogo_clientes AS catclient", "sell.cliente", "=", "catclient.id")
    ->where([
      "catclient.token_cat_clientes" => $token_cat_clientes,
    ...
    $movimientosClienteByAnticipoOrdenPago = MovimientosBancariosModelo::join("main_empresas AS emp", "fnzs_actividad_movimientos.empresa", "=", "emp.id")
    ->join("fnzs_pagos_pago AS pay", "fnzs_actividad_movimientos.pago", "=", "pay.id")
    ->join("fnzs_pagos_pago_ordenes_vinculadas AS ordpv", "pay.id", "=", "ordpv.pago_realizado")
    ->join("fnzs_pagos_orden AS order", "ordpv.orden_pago_vinculada", "=", "order.id")
    ->join("ingr_catalogo_clientes_anticipo AS ant", "order.anticipo_cliente", "=", "ant.uuid_anticipo")
    ->join("ingr_catalogo_clientes AS catclient", "ant.cliente", "=", "catclient.id")
    ->where([
      "catclient.token_cat_clientes" => $token_cat_clientes,
      "emp.empresa_token" => $empresa
    ])      
    ...
    $movimientosClienteDirecto = MovimientosBancariosModelo::join("main_empresas AS emp", "fnzs_actividad_movimientos.empresa", "=", "emp.id")
    ->join("fnzs_pagos_pago AS pay", "fnzs_actividad_movimientos.pago", "=", "pay.id")
    ->join("ingr_catalogo_clientes AS catclient", "pay.vinc_cliente", "=", "catclient.id")
    ...
    $movimientosClienteSellNotPag = DB::table('fnzs_pagos_orden as order')
    ...
    $querySaldoInicial = $movimientosClienteByCompraOrdenPago->unionAll($movimientosClienteByAnticipoOrdenPago)->unionAll($movimientosClienteDirecto)->unionAll($movimientosClienteSellNotPag)
  ...
  private function parteRelMovPagoDirecto($pago,$JwtAuth){
    ...
    ->leftJoin("ingr_catalogo_clientes AS catclient", "pag.vinc_cliente", "=", "catclient.id")
    ->leftJoin("sos_personas AS client_data", "catclient.cliente", "=", "client_data.id")
    ...
    ->select([
      'client_data.nombre_extendido AS prvName',
  ...
  private function parteRelMovPagoByOrden($pago,$JwtAuth){
    ...
    ->leftJoin("ingr_catalogo_clientes AS catclient", "order.ord_cliente", "=", "catclient.id")
    ->leftJoin("sos_personas AS client_data", "catclient.cliente", "=", "client_data.id")
    ...
    ->leftJoin("fnzs_catalogos_fed_estados_municipios AS fedEstSocial", "social_main.cliente_imss", "=", "fedEstSocial.id")

    ->leftJoin("cont_reg_fisc_declaraciones_imp_federales AS fedMain", "order.declaracion_imp_federales", "=", "fedMain.id")
    ->leftJoin("fnzs_catalogos_fed_estados_municipios AS fedEstImpFed", "fedMain.cliente_sat", "=", "fedEstImpFed.id")
  ...
  public function movimientosFinancieroscliente(Request $request){
    ...
    if ($validate->fails()) {
      ...
      $token_cat_clientes = $request->input('token_cat_clientes');
      ...
      $queryMovimientos = $movimientosClienteByCompraOrdenPago->unionAll($movimientosClienteByAnticipoOrdenPago)
      ->unionAll($movimientosClienteDirecto)->unionAll($movimientosClienteSellNotPag)
      ->orderBy('fecha_contabilizacion_movimiento', 'ASC')
      ->get();
      ...
      if ($queryMovimientos->isEmpty()) {
        ...
        $saldo_inicial_client = $this->saldoInicialClienteByToken($token_cat_clientes,$empresa,$fechaInicio);
        $saldo_acumulado_depositos = 0;
        $saldo_acumulado_retiros = 0;
        $saldo_acumulado_progresivo = $saldo_inicial_client;
        $contador = 0;
        ...
        foreach ($queryMovimientos as $vMov) {
          ...
          $token_cliente = $vMov->token_cat_clientes;
        ...
        $client_result_saldo = ($saldo_inicial_client + $saldo_acumulado_depositos) - $saldo_acumulado_retiros;
        ...
        $dataMensaje = array(
          ...
          "movimientos_saldo_inicial" => "$".number_format($saldo_inicial_client,$decimalesMoneda,'.', ','),
          ...
          "saldo_final" => "$".number_format($client_result_saldo,$decimalesMoneda,'.', ','),
