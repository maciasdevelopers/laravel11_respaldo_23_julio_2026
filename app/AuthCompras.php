...
class AuthCompras{
    ...
    public function moduleLoginCompras($codigo_acceso,$password,$firebase_token_movil,$firebase_token_web){
            ...
            if ($rMod->mantenimiento == FALSE && $rMod->acceso == TRUE) {
                ...
                if ($signup) {
                    ...
                    foreach($queryLogin as $user){
                        //echo $user->emp_token;
                        /*$permissionLogin = DB::select("SELECT users.login_permission AS login,users.outside_compras FROM main_usuarios AS users JOIN main_empresas AS emp 
                            WHERE emp.id = users.empresa AND (users.codigo_acceso = ? OR users.username = ?) 
                            AND users.password = ?",[$codigo_acceso,$codigo_acceso,$password]);*/
                            
                        $permissionLogin = DB::select("SELECT login_permission AS login,outside_compras FROM main_usuarios WHERE (codigo_acceso = ? OR username = ?) AND password = ?",
                            [$codigo_acceso,$codigo_acceso,$password]);
                            
                        if ($permissionLogin[0]->login == TRUE && $permissionLogin[0]->outside_compras == TRUE) {
